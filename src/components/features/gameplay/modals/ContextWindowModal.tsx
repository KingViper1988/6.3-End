...
const ContextWindowModal: React.FC<ContextWindowModalProps> = ({
    ...
    const { resolvedInterfaceMode } = useTheme();
    const isMobileMode = resolvedInterfaceMode === 'mobile';
    ...
    return (
        <AnimatePresence>
            {show && (
                <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/80 backdrop-blur-sm p-0 md:p-4 pointer-events-auto">
                    <motion.div
                        ...
                        className={`relative bg-stone-200 dark:bg-mystic-950 border border-stone-400 dark:border-slate-800 rounded-xl shadow-2xl overflow-hidden flex flex-col animate-presence ${isMobileMode ? '' : 'w-[99vw] h-[99vh]'}`}
                        style={isMobileMode ? {
                            zoom: 2.0,
                            width: '96vw',
                            height: '96vh',
            ...
            {showImportExport && (
                <div className="fixed inset-0 z-[170] flex items-center justify-center bg-black/85 backdrop-blur-sm p-4">
                    <div 
                        className="bg-stone-250 dark:bg-mystic-900 border border-stone-400 dark:border-slate-800 rounded-xl shadow-2xl p-5 space-y-4"
                        style={isMobileMode ? {
                            zoom: 2.0,
                            width: '90vw',
                            maxWidth: '90vw',
