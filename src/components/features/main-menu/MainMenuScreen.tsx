...
const MainMenuScreen: React.FC<NavigationProps> = ({ onNavigate, onGameStart }) => {
  ...
  const [bgBlur, setBgBlur] = useState<boolean>(dbService.getKeyValueSync('ark_v2_bg_blur') !== false && dbService.getKeyValueSync('ark_v2_bg_blur') !== 'false'); // Default to true
  ...
  const resetBg = async () => {
    ...
    dbService.removeKeyValue('ark_v1_custom_bg'); // Clean up legacy if exists
    dbService.removeKeyValue('ark_v2_bg_blur');
  ...
  const toggleBlur = () => {
    ...
    setBgBlur(newBlur);
    dbService.setKeyValue('ark_v2_bg_blur', newBlur);
    ...
    <div className="flex flex-col h-full w-full relative overflow-hidden">
      ...
      <div className="absolute top-4 left-4 z-50 flex flex-wrap gap-2 max-w-[calc(100%-2rem)]">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-emerald-500/20 hover:bg-emerald-500/40 rounded-xl text-emerald-400 shadow-lg backdrop-blur-md border border-emerald-500/50 transition-all active:scale-95 group"
